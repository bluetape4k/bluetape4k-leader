 ...
 * **동일 reference** 인지 확인합니다 (T8 PR 3, Issue #79).
 *
 * ## 검증 방식
 ...
 *
 * ## 검증 4 케이스
 * - sync single ([RedissonLeaderElector])
 * - suspend single ([RedissonSuspendLeaderElector])
 * - sync group ([RedissonLeaderGroupElector])
 * - suspend group ([RedissonSuspendLeaderGroupElector])
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class RedissonExtendDelegateReferenceTest: AbstractRedissonLeaderTest() {

    companion object: KLogging()

    private fun randomLockName(): String = "extdelref-rds-${Base58.randomString(8)}"
    ...
    @Test
    fun `sync group — extendActiveLockDetailed uses updateLeaseTime and Extended is returned`() {
        val elector = RedissonLeaderGroupElector(
            redissonClient,
    ...
    @Test
    fun `suspend group — extendActiveLockDetailedSuspend uses updateLeaseTimeAsync and Extended is returned`() =
        runSuspendIO {
            val elector = RedissonSuspendLeaderGroupElector(
                redissonClient,
                LeaderGroupElectionOptions(maxLeaders = 2),
            )
            val lockName = randomLockName()
