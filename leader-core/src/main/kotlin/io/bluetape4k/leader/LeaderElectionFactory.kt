...
/**
 * 호출 단위 옵션으로 [LeaderElection] 인스턴스를 생성하는 팩토리 SPI.
 *
 * ## 도입 배경
 * 코어 인터페이스 [LeaderElection.runIfLeader]는 `(lockName, action)` 시그니처만 제공하며 옵션을
 ...
 *
 * ## 책임 분리
 * - 본 SPI: 호출자가 요청한 옵션으로 새 [LeaderElection] 인스턴스 생성
 ...
 *
 * @see LeaderGroupElectionFactory 그룹 리더 선출용 팩토리
 */
fun interface LeaderElectionFactory {

    /**
     * 주어진 [options]로 새로운 [LeaderElection] 인스턴스를 생성한다.
     *
     * 동일 [options]에 대해 매번 새 인스턴스를 반환할 수 있으며, 동일성 보장은 호출자 책임이다.
     *
     * @param options 새 인스턴스에 적용할 옵션 (waitTime, leaseTime)
     * @return 호출 단위 옵션이 적용된 새 [LeaderElection] 인스턴스
     */
