...
/**
 * 호출 단위 옵션으로 [LeaderGroupElector] 인스턴스를 생성하는 팩토리 SPI.
 *
 * ## 도입 배경
 * 코어 인터페이스 [LeaderGroupElector.runIfLeader]는 `(lockName, action)` 시그니처만 제공하며
 ...
 * ```
 *
 * @see LeaderElectorFactory 단일 리더 선출용 팩토리
 */
fun interface LeaderGroupElectorFactory {

    /**
     * 주어진 [options]로 새로운 [LeaderGroupElector] 인스턴스를 생성한다.
     *
     * @param options 새 인스턴스에 적용할 옵션 (maxLeaders, waitTime, leaseTime)
     * @return 호출 단위 옵션이 적용된 새 [LeaderGroupElector] 인스턴스
     */
