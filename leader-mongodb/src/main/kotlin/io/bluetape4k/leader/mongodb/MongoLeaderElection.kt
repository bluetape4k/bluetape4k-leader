 ...
 */
class MongoLeaderElection private constructor(
    ...
    companion object : KLogging() {
        ...
        operator fun invoke(
            ...
            return MongoLeaderElection(collection, options)
        }
    }
...
fun <T> MongoCollection<Document>.runIfLeader(
    ...
    options: MongoLeaderElectionOptions = MongoLeaderElectionOptions.Default,
    action: () -> T,
): T? = MongoLeaderElection(this, options).runIfLeader(lockName, action)
...
fun <T> MongoCollection<Document>.runAsyncIfLeader(
    ...
    options: MongoLeaderElectionOptions = MongoLeaderElectionOptions.Default,
    action: () -> CompletableFuture<T>,
): CompletableFuture<T?> = MongoLeaderElection(this, options).runAsyncIfLeader(lockName, executor, action)
