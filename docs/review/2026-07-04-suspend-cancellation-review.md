...
3. 일시 중단 전략적 선출기는 `runCatching`를 통해 `updateResult` 실패를 기록했으며, 이는 성공 또는 실패를 기록하는 동안 취소를 삼킬 수도 있습니다.
4. 동기식 전략 선출기는 여전히 `runCatching`를 사용하지만 정지 API를 호출하지 않으며 이 검토의 코루틴 취소 범위를 벗어납니다.
