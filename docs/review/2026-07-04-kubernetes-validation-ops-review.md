...
- `leader-k8s`: 네임스페이스 및 이름 유효성 검사 계약을 임대합니다.
- `examples/k8s-lease`: Fabric8 임대 호출 전 클라이언트 측 검증.
...
1. 정확성: 통과
   ...
   - 예 이제 Fabric8이 호출을 생성, 읽기, 업데이트 또는 삭제하기 전에 임대 이름이 빠르게 failure합니다.
...
4. 백엔드 소유권 안전성: 통과
   - 잘못된 네임스페이스 및 임대 이름은 Kubernetes로 전달되는 대신 클라이언트 측에서 거부됩니다.
   - 운영자 런타임 역할은 더 이상 임대 `delete`를 부여하지 않습니다.

5. 테스트: 합격
   - 잘못된 네임스페이스 및 임대 이름에 대한 부정적인 테스트를 추가했습니다.
