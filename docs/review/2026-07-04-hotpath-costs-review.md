...
- Consul 그룹 선출은 여전히 포화 경합 하에서 `null`를 반환하고 후보 세션을 해제합니다.
- 이제 필수 클레임 인덱스를 생성할 수 없는 경우 웹후크 폴러 시작이 실패합니다.
...
- Consul 위임 테스트 블로킹 및 일시 중단은 `maxLeaders = 64`를 사용하여 제한된 획득 호출 수를 검증합니다.
