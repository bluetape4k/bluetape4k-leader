...
- **`override fun close()`**: scope 취소 후 indexBuildJob 완료 대기 — Spring `@PreDestroy` / `DisposableBean` 연동 가능
  ```kotlin
  override fun close() {
      runCatching { scope.cancel() }
      runCatching { runBlocking { indexBuildJob?.join() } }
  }
  ```
- index 생성:
...
- `LeaderHistorySink`에 `fun deleteOlderThan(cutoff: Instant, limit: Int = 1000): Int` default method 추가 (noop 기본 구현)
- `SuspendLeaderHistorySink`에 대응하는 suspend 버전 추가
- auto-config 조건: `@ConditionalOnProperty(name = ["bluetape4k.leader.history.retention.enabled"], matchIfMissing = true)`
- KDoc (English): retention period, chunk size, cron 설정 예시 포함
...
| T-Retention | Retention 배치 삭제 (`@Scheduled`, 30일/1000-row 청크) | medium | leader-spring-boot | T6, T32 |
...
**Total**: 57 tasks (Rev 2: +3 신규 — T20-BC, T-Retention, H3~H7 반영)
- high: 23
- medium: 24 (+3: T20-BC, T-Retention + T34 확장)
- low: 10
