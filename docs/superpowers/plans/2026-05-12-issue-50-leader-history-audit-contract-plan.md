...
- **`## Security / Trust Boundary`** KDoc 섹션 추가 (Round 3 Sec-1): "`token` 필드는 live lock release 권한을 가진 credential임. `toString()` 기본 구현에 포함되므로 log/외부 노출 주의. audit DB read access는 lock backend와 동일 trust boundary로 제한해야 함. 필요 시 `tokenFingerprint = Base64(SHA-256(token)).take(22)` 별도 컬럼 사용."
...
- `recordFailed(error: Throwable?)`:
  ...
  - **중요 제한사항 KDoc 명시** (Round 3 Sec-2): "`sanitizeForLog()`는 control character / log-injection 방지용이며 credential 스크러빙이 아님. JDBC/driver 예외 메시지에 비밀번호나 커넥션 URL이 포함될 수 있음. 호출자는 elector에 credential-bearing 예외가 전달되기 전에 메시지를 정제할 책임이 있음."
...
내용 (H1 수정):
- `open class MicrometerSafeLeaderHistoryRecorder(sink: LeaderHistorySink, private val meterRegistry: MeterRegistry) : SafeLeaderHistoryRecorder(CounterAwareSinkDecorator(sink, meterRegistry, sink::class.simpleName ?: "unknown"))` (Round 3 MEDIUM: `open` 수식어 추가로 blocking/suspend 대칭)
...
- `open class MicrometerSuspendSafeLeaderHistoryRecorder` — T13과 동일하게 `open` (Round 3 MEDIUM: 대칭 일치)
- T36-A (suspend test file) 추가로 counter 검증
- KDoc (English)
...
- `CHANGELOG.md`에 `### BREAKING CHANGES` 항목 추가 (Round 3 User-1: before/after 코드 예제 필수):
  ```
...
### T36-A: leader-micrometer 단위 테스트 — blocking + suspend (Round 3 H3 통합)
**complexity: medium**
**모듈**: `leader-micrometer`
**파일**:
- `leader-micrometer/src/test/kotlin/.../history/MicrometerSafeLeaderHistoryRecorderTest.kt` (신규, blocking)
- `leader-micrometer/src/test/kotlin/.../history/MicrometerSuspendSafeLeaderHistoryRecorderTest.kt` (신규, suspend — Round 3 H3 추가)

테스트 케이스 (blocking, spec §7-1):
...
테스트 케이스 (suspend, T13-S 대응 — Round 3 H3):
- 위 5개 시나리오의 `runTest` 버전 (`coEvery / coVerify`)
- CE rethrow: suspend sink throws CE → `assertFailsWith<CancellationException>` (counter increment 없음)
- sink simpleName in tag 검증

도구: `SimpleMeterRegistry`, MockK, `runTest`
...
**Total**: 62 tasks (Rev 4: Round 3 HIGH 3건 + MEDIUM 4건 반영 — 태스크 수 변동 없음)
- high: 25 (+2: T13-S)
...
> - Round 3 H3: T36-A suspend 테스트 파일(`MicrometerSuspendSafeLeaderHistoryRecorderTest`) 명시
> - Round 3 Medium: T13/T13-S `open` 대칭, T25 limit KDoc+T37-C 테스트, T4 Security KDoc, T10 credential KDoc
> - Round 3 User-1: T20-BC before/after 코드 예제 추가
>
