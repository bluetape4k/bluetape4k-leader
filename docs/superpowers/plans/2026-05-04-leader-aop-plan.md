...
| ~~T2.8a~~ | **폐기** — `leader-core` 의 기존 `LeaderElectionException` / `LeaderGroupElectionException` 재사용 (`leader-core/src/main/kotlin/io/bluetape4k/leader/LeaderExceptions.kt`). 신규 wrapper 클래스 미정의 | — | — | 사용자 지적 (2026-05-04): 기존 예외 재사용 |
...
| **T5.9c NEW** [Step 3-P-Sec-3][R-33] | `LeaderElectionException` / `LeaderGroupElectionException` wrapping 테스트 (기존 예외 재사용) — RETHROW 모드 backend (Lettuce/Redisson/Mongo) throw 시 wrapped 기존 예외, message 일반화 (host/credentials 미포함), cause 보존. group Aspect 는 `LeaderGroupElectionException` 사용 검증 | medium | `.../aop/AspectExceptionWrappingTest.kt` | infra disclosure 회귀 |
...
- [x] ~~T2.8a `LeaderBackendException` 정의~~ → 폐기. `leader-core` 기존 `LeaderElectionException`/`LeaderGroupElectionException` 재사용
