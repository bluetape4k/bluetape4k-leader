...
- 통과: 슬라이스는 좁은 상태로 유지됩니다. 단일 리더 런타임 슬라이스 전에는 그룹 선출, Spring, Ktor 또는 런타임 선출기 계약이 도입되지 않습니다.
...
- PASS: 이 슬라이스는 아직 차단 런타임 선출기를 추가하지 않습니다. 생산 루프, 스케줄러 또는 갱신 스레드가 도입되지 않습니다.
...
- 통과: 문서에서는 백엔드에 미리보기라는 라벨을 지정하고 런타임 선출기가 아직 진행 중이라고 명시합니다.
