...
- 통과: 세션 범위 릴리스는 선택된 Consul 세션을 파괴하여 취소 또는 작업 failure 후 오래된 잠금 소유권을 방지합니다.
- 통과: ACL 토큰 또는 엔드포인트 자격 증명 자료가 일시 중단 API 표면에 의해 기록되거나 방출되지 않습니다.
...
- 통과: 일반 잠금 경합은 `null`를 반환하고, 선거 후 작업 failure는 `LeaderRunResult.ActionFailed`에 매핑됩니다.
...
- PASS: `state()` 동작은 차단 및 일시 중단 지원 Consul 리더 런타임 표면 간에 정렬됩니다.

조사 결과: 없음 P0/P1.

## 계층 5 - 테스트 / 유형 / 자동 failure

- 통과: 테스트에서는 경합 건너뛰기, 취소 정리, 작업 failure 정리, 대기 시간 초과 정리, 대기 후보 갱신 failure, 자동 확장 갱신, 일시 중지 잠금 어설션/확장 동작, 상태 매핑 및 확장 기능 동작을 다루고 있습니다.
- 통과: 통합 테스트는 실제 Consul에 대한 취소 및 작업 failure 후 다시 획득됨을 입증합니다.
...
- Consul 그룹 선택, Spring 자동 구성 및 이벤트/메트릭 표면이 포함된 이슈 #345를 계속하세요.
