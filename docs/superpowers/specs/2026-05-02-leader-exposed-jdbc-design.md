...
- 재시도 루프: `tryLock` 내부에서 deadline까지 UPDATE → INSERT → retryStrategy.sleep 반복 (sleep은 트랜잭션 바깥)
...
- `retryStrategy.sleep(remaining)` 호출은 **반드시 `transaction {}` 블록 바깥**에서 실행 (Section 3.1 시퀀스 참조)
