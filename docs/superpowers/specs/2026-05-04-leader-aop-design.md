...
- **본 PR 신규 module 2개 — flat naming** [M3]:
  - `leader-spring-boot3 (merged)` (Spring AOP 런타임 프록시)
  - `leader-spring-boot4 (merged)` (AspectJ 빌드 시 도구)
  - artifact ID: `io.github.bluetape4k.leader:leader-spring-boot3 (merged)` / `...:leader-spring-boot4 (merged)` (flat naming 일관성)
  - `settings.gradle.kts` 에 `include(":leader-spring-boot3 (merged)", ":leader-spring-boot4 (merged)")` 추가
...
| `leader-spring-boot3 (merged)` | common + `spring-aop:6.2.x` (Boot 3 BOM `runtime`) | 본 PR 신규 module — flat naming [M3]. Boot 3 BOM 으로 Spring 6.2 lock |
| `leader-spring-boot4 (merged)` | common + `spring-aspects:7.x` + `aspectjweaver` + Freefair Gradle 플러그인 (Boot 4 BOM `runtime`) | 본 PR 신규 module — flat naming [M3]. Boot 4 BOM 으로 Spring 7.x runtime 제공 |
...
```kotlin
include(":leader-spring-boot3 (merged)")
include(":leader-spring-boot4 (merged)")
```
...
### 4.4 자동 구성 (`leader-spring-boot3 (merged)`, `leader-spring-boot4 (merged)`) [M3]
...
**Factory 빈 등록 위치 = AOP autoconfig** (`leader-spring-boot3 (merged)` / `-boot4-aspectj` 의 `LeaderAopAutoConfiguration`). 각 backend 모듈은 factory **클래스만** 정의 (Spring 의존 없음, 순수 Kotlin 유지). 실제 `@Bean` 등록은 AOP autoconfig 가 담당하며, `@ConditionalOnBean(RedissonClient::class)` 등으로 사용자가 backend client bean 등록 시에만 factory 활성화 [Q-P1 (b)].
...
- **신규 module 2개 — flat naming** [M3]: `leader-spring-boot3 (merged)`, `leader-spring-boot4 (merged)` (`settings.gradle.kts` include 추가)
...
- [ ] `leader-spring-boot3 (merged)` 모듈 + `LeaderAopAutoConfiguration` + `AutoConfiguration.imports` + **`@ConditionalOnBean(LeaderElectionFactory)` factory-only 고정** + `@ConditionalOnProperty` [H2][R-29][M3]
- [ ] `leader-spring-boot4 (merged)` 모듈 + Freefair AspectJ Gradle 플러그인 + 동일 자동설정 [M3]
- [ ] **`settings.gradle.kts` 신규 module include** — `:leader-spring-boot3 (merged)`, `:leader-spring-boot4 (merged)` [M3]
...
- [ ] `CLAUDE.md` Repository Layout 갱신 (신규 모듈 `leader-spring-boot3 (merged)`/`leader-spring-boot4 (merged)` 추가, AOP 사용 가이드)
...
- **Codex 2nd round 7건 (사용자 승인)**:
  ...
  - **M3**: 신규 module **flat naming** — `leader-spring-boot3 (merged)`, `leader-spring-boot4 (merged)` (`settings.gradle.kts` include)
- **FINAL 결정 (사용자 승인)**:
