...
| 모듈 | 의존성 | 비고 |
|------|--------|------|
| `leader-spring-boot-common` | `spring-context`, `spring-aop`, `spring-expression`, Caffeine | sync only — `kotlinx-coroutines-reactor` 의존성 **제거** [#80] |
| `leader-spring-boot3-aop` | common + `spring-aop` (Boot 3 BOM) | 본 PR 신규 module — flat naming [M3] |
| `leader-spring-boot4-aspectj` | common + `spring-aspects` + `aspectjweaver` + Freefair Gradle 플러그인 | 본 PR 신규 module — flat naming [M3] |
...
| `LockNameValidator` | length / charset 검증 (R-4) |
...
| `LeaderAopAutoConfiguration` | `@AutoConfiguration` + `@ConditionalOnClass(Aspect::class)` + **`@ConditionalOnBean(LeaderElectionFactory::class)` factory-only 고정** [H2][R-29] + `@ConditionalOnProperty(name = "bluetape4k.leader.aop.enabled", matchIfMissing = true)` + `@EnableAspectJAutoProxy(proxyTargetClass = true)` [R-9]. 내부 빈 메서드 모두 `@Role(ROLE_INFRASTRUCTURE)`. **사용자가 `LeaderElection` 빈만 직접 등록하고 factory 미등록 시 AOP 비활성** — README 명시 + 마이그레이션 가이드. 6 backend factory 가 본 PR scope 에 포함되므로 backend autoconfig 가 활성화된 사용자는 factory 도 자동 등록됨 |
...
| `META-INF/spring/...AutoConfiguration.imports` | `LeaderAopAutoConfiguration` FQCN |
...
| Exposed JDBC | `leader-exposed` | `exposedJdbcLeaderElectionFactory`, `exposedJdbcLeaderGroupElectionFactory` |
...
각 백엔드 모듈은 `LeaderElection` 빈 등록 시 동일한 connection/client 자원을 공유하는 factory 도 자동 등록.
...
fun around(pjp: ProceedingJoinPoint): Any? {
    ...
    metrics.forEach { it.onLockAttempt(lockName, opts) }
    ...
    return try {
        ...
        if (result == null) {
            metrics.forEach { it.onLockNotAcquired(lockName, opts, SkipReason.CONTENTION) }
        } else {
            metrics.forEach { it.onTaskFinished(lockName, (System.nanoTime() - start).nanoseconds) }
            warnIfNearLease(start, opts.leaseTime)  // [H10]
        }
        result  // T? — null이면 미선출 또는 본문 null (best-effort 한계)
    } catch (e: Throwable) {
        metrics.forEach { it.onTaskFailed(lockName, (System.nanoTime() - start).nanoseconds, e) }
        when (ann.failureMode) {
            LeaderAspectFailureMode.RETHROW -> throw e
            LeaderAspectFailureMode.SKIP -> {
                log.warn(e) { "leader.aop.skipped lockName=$lockName reason=BACKEND_ERROR" }
                metrics.forEach { it.onLockNotAcquired(lockName, opts, SkipReason.BACKEND_ERROR) }
                null
            }
        }
    }
...
4. **`SimpleEvaluationContext` 평가** — `forPropertyAccessors(DataBindingPropertyAccessor.forReadOnlyAccess()).withMethodResolvers(DataBindingMethodResolver.forInstanceMethodInvocation()).build()`
...
`T(...)` 타입 참조 표현식 차단, Spring 빈 참조 표현식 차단. read-only property access만 허용.
...
- 어노테이션 (`@LeaderElection`, `@LeaderGroupElection`, **`@Target(METHOD)` 직접 부착만** — 메타 어노테이션 컴포지션 [#84] 후속 [C2][R-27]) + `LeaderAspectFailureMode { RETHROW, SKIP }` enum
...
- [ ] **`@Target(METHOD)` 직접 부착만 매칭 검증** — 메타 어노테이션 (`@JobLock`) 시 advice 미적용 확인 [C2][R-27][#84]
