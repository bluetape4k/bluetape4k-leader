...
지원되는 단일 리더 백엔드에서 `autoExtend=true`를 설정하면 공통 watchdog이 시작됩니다. 선출된 호출 범위 안에서는 `LockExtender.extendActiveLockDetailed()` 또는 suspend variant를 직접 호출할 수도 있습니다. 0.4.0에서 그룹 자동 연장은 일반 계약이 아닙니다.
