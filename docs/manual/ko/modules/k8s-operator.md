...
# Kubernetes 오퍼레이터 리더 선출 워크숍

> 실행 가능한 안정판 워크숍 · Kubernetes Lease와 Spring scheduling · `0.4.0` 소스 기준

## 제공하는 기능 {#problem}

모든 오퍼레이터 복제본은 주기 신호를 받지만, `cronjob-reconciler` Lease를 가진 파드만 사용자 정의 리소스를 조정하는 본문을 실행해야 합니다.
...
- **조정 백엔드:** Kubernetes Lease가 소유권을 관리하고 Spring 스케줄링이 주기 작업을 시작합니다.
...
빌드 성공만 보고 끝내지 말고 출력과 테스트에서 다음 동작을 확인하세요. 대기 중인 파드도 계속 주기 신호를 받지만 작업 본문은 실행하지 않고, 한 파드만 리소스를 조정해야 합니다. 리스를 잃으면 다른 파드가 인계해야 합니다.
...
두 개 이상의 후보를 함께 실행하세요. 대기 중인 파드도 계속 주기 신호를 받지만 작업 본문은 실행하지 않고, 한 파드만 리소스를 조정하는지 확인합니다. 리스를 잃었을 때 다른 파드가 인계하는지도 살펴봅니다.

## 작업별 API {#api-by-task}

1. `OperatorController`에서 애플리케이션이 호출할 조정 경계를 확인합니다.
2. `K8sOperatorApp`에서 클라이언트 생성, 후보 구성, 결과 출력 순서를 따라갑니다.
...
이 워크숍은 Kubernetes Lease와 Spring 스케줄링을 실행 가능한 Gradle 애플리케이션에 연결합니다. 예제는 학습에 필요한 로컬 실행 환경을 준비합니다. 운영에서는 백엔드 클라이언트를 주입하고 애플리케이션 생명주기에 맞춰 닫아야 합니다.

스케줄러, HTTP 계층, 오퍼레이터, 코루틴 프레임워크는 조정 경계를 호출하는 역할만 맡아야 합니다. 락 획득을 무관한 업무 서비스 안에 숨기지 않으면 경쟁과 인계 상태를 테스트와 메트릭에서 확인하기 쉽습니다.

## 설정 {#configuration}

Lease 네임스페이스와 이름, 파드 식별자, 스케줄러 지연 시간, RBAC 권한, Deployment 복제본 수, Kubernetes 클라이언트 설정을 먼저 확인하세요. 같은 작업을 맡는 모든 인스턴스는 같은 논리 락 이름을 써야 하고, 서로 무관한 작업이나 환경은 다른 이름을 써야 합니다.
