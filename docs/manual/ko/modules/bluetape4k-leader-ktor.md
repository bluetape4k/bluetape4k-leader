...
`code`, `message`, 숫자 `status`만 포함합니다. status override도 같은 allow-list를
사용합니다. Backend message, stack trace, cause 상세는 정제한 cause type만 로그에
남기고 응답에 복사하지 않습니다. `CancellationException`은 다시 던집니다.
...
HTTP로 보내지 않습니다. Plugin이 정제한 `WARN`을 기록하고 실패한 회차만 건너뛴 뒤
다음 주기를 계속합니다.
