...
- **조정과 관측:** Lettuce Redis가 소유권을 관리하고 Spring Boot, Micrometer, Prometheus, Grafana가 실행 결과를 수집하고 보여줍니다.
...
빌드 성공만 보고 끝내지 말고 출력과 테스트에서 다음 동작을 확인하세요. `leader_aop_*` 시계열이 나타나고 경쟁할 때 획득과 건너뛰기 비율이 바뀌며, 작업이 끝나면 활성 게이지가 0으로 돌아와야 합니다.
...
두 개 이상의 후보를 함께 실행하세요. `leader_aop_*` 시계열이 나타나고 경쟁할 때 획득과 건너뛰기 비율이 바뀌며, 작업이 끝나면 활성 게이지가 0으로 돌아오는지 확인합니다. 그런 다음 현재 소유자를 종료하거나 락을 반환해 다음 주기의 인계까지 살펴봅니다.

## 작업별 API {#api-by-task}

1. `PrometheusDashboardApp`에서 애플리케이션이 호출할 조정 경계를 확인합니다.
2. `PrometheusDashboardApp`에서 클라이언트 생성, 후보 구성, 결과 출력 순서를 따라갑니다.
...
이 워크숍은 Spring Boot와 Lettuce Redis의 선출 결과를 Micrometer, Prometheus, Grafana에서 관찰하는 방법을 보여줍니다. 예제는 학습에 필요한 로컬 실행 환경을 준비합니다. 운영에서는 백엔드 클라이언트를 주입하고 애플리케이션 생명주기에 맞춰 닫아야 합니다.

스케줄러, HTTP 계층, 오퍼레이터, 코루틴 프레임워크는 조정 경계를 호출하는 역할만 맡아야 합니다. 락 획득을 무관한 업무 서비스 안에 숨기지 않으면 경쟁과 인계 상태를 테스트와 메트릭에서 확인하기 쉽습니다.

## 설정 {#configuration}

Actuator 노출 범위, Redis URL, 락 이름, 메트릭 태그, Prometheus 수집 대상, Grafana 인증 정보, 호스트 포트 연결을 먼저 확인하세요. 같은 작업을 맡는 모든 인스턴스는 같은 논리 락 이름을 써야 하고, 서로 무관한 작업이나 환경은 다른 이름을 써야 합니다.
