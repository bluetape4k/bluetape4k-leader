...
두 소유자 식별자가 하나의 Kubernetes Lease를 두고 경쟁합니다. 현재 소유자는 Lease를 갱신하거나 반환하고, 경쟁자는 소유권이 만료되거나 반환될 때까지 `CONFLICT`를 받습니다.
...
- **조정 백엔드:** Kubernetes `coordination.k8s.io/v1` Lease 객체가 리더 선출 상태와 소유권을 관리합니다.
...
2. `K8sLeaseLeaderElectionExample`에서 클라이언트 생성, 후보 구성, 결과 출력 순서를 따라갑니다.
...
이 워크숍은 Kubernetes `coordination.k8s.io/v1` Lease를 실행 가능한 Gradle 애플리케이션에 연결합니다. 예제는 학습에 필요한 로컬 실행 환경을 준비합니다. 운영에서는 백엔드 클라이언트를 주입하고 애플리케이션 생명주기에 맞춰 닫아야 합니다.

스케줄러, HTTP 계층, 오퍼레이터, 코루틴 프레임워크는 조정 경계를 호출하는 역할만 맡아야 합니다. 락 획득을 무관한 업무 서비스 안에 숨기지 않으면 경쟁과 인계 상태를 테스트와 메트릭에서 확인하기 쉽습니다.

## 설정 {#configuration}

네임스페이스, Lease 이름, 소유자 식별자, RBAC, API 연결, 리스 시간을 먼저 확인하세요. 같은 작업을 맡는 모든 인스턴스는 같은 논리 락 이름을 써야 하고, 서로 무관한 작업이나 환경은 다른 이름을 써야 합니다.
