...
> 실행 가능한 안정판 워크숍 · Apache Curator를 사용하는 ZooKeeper 세션 락 · `0.4.0` 소스 기준
...
- **조정 백엔드:** Apache Curator를 사용하는 ZooKeeper 세션 락이 리더 선출 상태와 소유권을 관리합니다.
...
빌드 성공만 보고 끝내지 말고 출력과 테스트에서 다음 동작을 확인하세요. 노드 A가 세션 락을 가진 동안에는 A만 실행되고 노드 B는 건너뛰며, A가 락을 반환한 뒤 B가 실행되어야 합니다.

## 빠르게 시작하기 {#quick-start}

사전 준비: JDK 21 이상. 백엔드 테스트가 Testcontainers를 사용한다면 Docker도 필요합니다.

```bash
./gradlew :examples:zookeeper-scheduler:run
```

두 개 이상의 후보를 함께 실행하세요. 노드 A가 세션 락을 가진 동안에는 A만 실행되고 노드 B는 건너뛰며, A가 락을 반환한 뒤 B가 실행되는지 확인합니다. 그런 다음 현재 소유자를 종료해 다음 주기의 인계까지 살펴봅니다.

## 작업별 API {#api-by-task}

1. `ZooKeeperLegacyScheduler`에서 애플리케이션이 호출할 조정 경계를 확인합니다.
2. `ZooKeeperSchedulerDemo`에서 클라이언트 생성, 후보 구성, 결과 출력 순서를 따라갑니다.
...
이 워크숍은 Apache Curator 기반 ZooKeeper 세션 락을 실행 가능한 Gradle 애플리케이션에 연결합니다. 예제는 학습에 필요한 로컬 실행 환경을 준비합니다. 운영에서는 백엔드 클라이언트를 주입하고 애플리케이션 생명주기에 맞춰 닫아야 합니다.

스케줄러, HTTP 계층, 오퍼레이터, 코루틴 프레임워크는 조정 경계를 호출하는 역할만 맡아야 합니다. 락 획득을 무관한 업무 서비스 안에 숨기지 않으면 경쟁과 인계 상태를 테스트와 메트릭에서 확인하기 쉽습니다.

## 설정 {#configuration}

Curator 연결 문자열과 생명주기, 기본 경로, 노드 식별자, 락 이름, 실행 일정 식별자, 세션 제한 시간, 재시도 정책을 먼저 확인하세요. 같은 작업을 맡는 모든 인스턴스는 같은 논리 락 이름을 써야 하고, 서로 무관한 작업이나 환경은 다른 이름을 써야 합니다.
...
ZooKeeper 소유권은 TTL 갱신 루프가 아니라 Curator 세션을 따릅니다. 세션 상태와 정상적인 클라이언트 종료를 운영에서 관리해야 합니다.
...
### ZooKeeper scheduler 아키텍처

[![ZooKeeper scheduler 아키텍처](../../assets/readme-diagrams/examples-zookeeper-scheduler-architecture-01.png)](../../assets/readme-diagrams/examples-zookeeper-scheduler-architecture-01.svg)

_배포본 README: [`examples/zookeeper-scheduler/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/zookeeper-scheduler/README.ko.md)_

### ZooKeeper scheduler 처리 흐름

[![ZooKeeper scheduler 처리 흐름](../../assets/readme-diagrams/examples-zookeeper-scheduler-flow-01.png)](../../assets/readme-diagrams/examples-zookeeper-scheduler-flow-01.svg)

_배포본 README: [`examples/zookeeper-scheduler/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/zookeeper-scheduler/README.ko.md)_

### ZooKeeper scheduler 실행 시나리오 다이어그램

[![ZooKeeper scheduler 실행 시나리오 다이어그램](../../assets/readme-diagrams/examples-zookeeper-scheduler-scenario-01.png)](../../assets/readme-diagrams/examples-zookeeper-scheduler-scenario-01.svg)

_배포본 README: [`examples/zookeeper-scheduler/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/zookeeper-scheduler/README.ko.md)_

### ZooKeeper scheduler 시퀀스 다이어그램

[![ZooKeeper scheduler 시퀀스 다이어그램](../../assets/readme-diagrams/examples-zookeeper-scheduler-sequence-01.png)](../../assets/readme-diagrams/examples-zookeeper-scheduler-sequence-01.svg)
