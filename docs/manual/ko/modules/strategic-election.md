...
> 실행 가능한 안정판 워크숍 · 백엔드와 무관한 로컬 전략 선출 · `0.4.0` 소스 기준

## 제공하는 기능 {#problem}

락에 먼저 도착한 노드가 항상 가장 적합한 리더는 아닙니다. 이 예제는 준비 상태, 성공률, 유휴 시간에 가중치를 적용해 후보를 평가한 뒤 승자의 본문만 실행합니다.
...
- **후보 선택:** 백엔드와 무관한 로컬 전략 선출기가 후보 점수를 비교해 승자를 결정합니다.
...
빌드 성공만 보고 끝내지 말고 출력과 테스트에서 다음 동작을 확인하세요. 정규화된 점수로 승자를 설명할 수 있어야 하며, 나머지 후보는 작업을 실행하지 않고 건너뛴 결과를 내야 합니다.
...
데모에서 두 개 이상의 후보를 함께 실행하세요. 정규화된 점수로 승자를 설명할 수 있고 나머지 후보는 작업을 실행하지 않은 채 건너뛴 결과를 내는지 확인합니다. 그런 다음 현재 소유자를 종료하거나 락을 반환해 다음 주기의 인계까지 살펴봅니다.

## 작업별 API {#api-by-task}

1. `StrategicElectionDemo`에서 애플리케이션이 호출할 조정 경계를 확인합니다.
2. `StrategicElectionDemo`에서 클라이언트 생성, 후보 구성, 결과 출력 순서를 따라갑니다.
...
이 워크숍은 백엔드와 무관한 로컬 전략 선출을 실행 가능한 Gradle 애플리케이션으로 보여줍니다. 예제는 학습에 필요한 로컬 실행 환경을 준비합니다. 운영에서는 백엔드 클라이언트를 주입하고 애플리케이션 생명주기에 맞춰 닫아야 합니다.

스케줄러, HTTP 계층, 오퍼레이터, 코루틴 프레임워크는 조정 경계를 호출하는 역할만 맡아야 합니다. 락 획득을 무관한 업무 서비스 안에 숨기지 않으면 경쟁과 인계 상태를 테스트와 메트릭에서 확인하기 쉽습니다.

## 설정 {#configuration}

후보 집합, 점수 계산기의 입력값, 정책을 드러내는 가중치, 동점 처리, 안정적인 노드 식별자를 먼저 확인하세요. 같은 작업을 맡는 모든 인스턴스는 같은 논리 락 이름을 써야 하고, 서로 무관한 작업이나 환경은 다른 이름을 써야 합니다.
...
예제 선출기는 로컬 구현이므로 서로 다른 프로세스를 조정하지 않습니다. JVM 경계를 넘어 승자를 강제해야 한다면 분산 백엔드를 결합해야 합니다.
...
### Strategic Election 아키텍처

[![Strategic Election 아키텍처](../../assets/readme-diagrams/examples-strategic-election-architecture-01.png)](../../assets/readme-diagrams/examples-strategic-election-architecture-01.svg)

_배포본 README: [`examples/strategic-election/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/strategic-election/README.ko.md)_

### Strategic election 처리 흐름

[![Strategic election 처리 흐름](../../assets/readme-diagrams/examples-strategic-election-flow-01.png)](../../assets/readme-diagrams/examples-strategic-election-flow-01.svg)

_배포본 README: [`examples/strategic-election/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/strategic-election/README.ko.md)_

### Strategic election 실행 시나리오 다이어그램

[![Strategic election 실행 시나리오 다이어그램](../../assets/readme-diagrams/examples-strategic-election-scenario-01.png)](../../assets/readme-diagrams/examples-strategic-election-scenario-01.svg)

_배포본 README: [`examples/strategic-election/README.ko.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/strategic-election/README.ko.md)_

### Strategic Election 실행 흐름

[![Strategic Election 실행 흐름](../../assets/readme-diagrams/examples-strategic-election-sequence-01.png)](../../assets/readme-diagrams/examples-strategic-election-sequence-01.svg)
