...
title: "Consul maintenance workshop"
locale: "en"
kind: "example"
...
# Consul maintenance workshop

> Runnable release workshop · Consul Session and KV · source pinned to `0.4.0`

## Problem {#problem}

Only one service instance should drain traffic, flush in-flight requests, and rotate an endpoint at a time. A Consul Session owns the KV lock for the duration of the maintenance body.
...
Use this workshop when only one service instance should drain traffic, flush in-flight requests, and rotate an endpoint at a time. A Consul Session owns the KV lock for the duration of the maintenance body. Start with the example unchanged, then replace only the workload while preserving the lock identity, bounded execution, and observable skip path.
...
- **Coordination backend:** Consul Session and KV stores or enforces the leadership decision.
...
Do not stop at a green build. Check this observable result: one coordinator completes the maintenance steps, a contender skips, and the contender can acquire after release.
...
Run two or more contenders when the demo supports it. Check this result: one coordinator completes the maintenance steps, a contender skips, and the contender can acquire after release. Then stop or release the current owner and run the next cycle to verify takeover.

## API by task {#api-by-task}

1. Read `ServiceMaintenanceCoordinator` first; it defines the application-facing coordination boundary.
2. Follow `ConsulMaintenanceDemo` to see client construction, contender setup, and result reporting.
3. Read `ServiceMaintenanceCoordinatorTest` to learn which ownership transitions the release promises.
...
The workshop integrates Consul Session and KV with a runnable Gradle application. The example owns its local fixture for learning, but production code should inject the backend client and let the application lifecycle close it.
...
Review Consul endpoint, datacenter, ACL token, network policy, `nodeId`, and the shared maintenance lock name. Use the same logical lock identity on every replica and different identities for unrelated jobs or environments.
...
[![Consul Maintenance Architecture diagram](../../assets/readme-diagrams/examples-consul-maintenance-architecture-01.png)](../../assets/readme-diagrams/examples-consul-maintenance-architecture-01.svg)
...
[![Consul maintenance flow diagram](../../assets/readme-diagrams/examples-consul-maintenance-flow-01.png)](../../assets/readme-diagrams/examples-consul-maintenance-flow-01.svg)

_Release README: [`examples/consul-maintenance/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/consul-maintenance/README.md)_

### Consul maintenance scenario diagram

[![Consul maintenance scenario diagram](../../assets/readme-diagrams/examples-consul-maintenance-scenario-01.png)](../../assets/readme-diagrams/examples-consul-maintenance-scenario-01.svg)

_Release README: [`examples/consul-maintenance/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/consul-maintenance/README.md)_

### Consul Maintenance Sequence Flow diagram

[![Consul Maintenance Sequence Flow diagram](../../assets/readme-diagrams/examples-consul-maintenance-sequence-01.png)](../../assets/readme-diagrams/examples-consul-maintenance-sequence-01.svg)
...
- [Stable example README](../../../../examples/consul-maintenance/README.md)
- [Gradle build](../../../../examples/consul-maintenance/build.gradle.kts)
- [Main implementation](../../../../examples/consul-maintenance/src/main/kotlin/io/bluetape4k/leader/examples/consulmaintenance/ServiceMaintenanceCoordinator.kt)
- [Demo entry point](../../../../examples/consul-maintenance/src/main/kotlin/io/bluetape4k/leader/examples/consulmaintenance/ConsulMaintenanceDemo.kt)
- [Verification test](../../../../examples/consul-maintenance/src/test/kotlin/io/bluetape4k/leader/examples/consulmaintenance/ServiceMaintenanceCoordinatorTest.kt)
