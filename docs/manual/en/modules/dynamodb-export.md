...
id: "dynamodb-export"
title: "DynamoDB export workshop"
locale: "en"
kind: "example"
...
# DynamoDB scheduled export workshop

> Runnable release workshop · DynamoDB conditional writes · source pinned to `0.4.0`

## Problem {#problem}

Two AWS service replicas receive the same billing export trigger. Conditional-write leadership selects one replica to write the export row while the contender returns `SKIPPED`.
...
Use this workshop when two AWS service replicas receive the same billing export trigger. Conditional-write leadership selects one replica to write the export row while the contender returns `SKIPPED`. Start with the example unchanged, then replace only the workload while preserving the lock identity, bounded execution, and observable skip path.
...
- **Coordination backend:** DynamoDB conditional writes stores or enforces the leadership decision.
...
Do not stop at a green build. Check this observable result: one `COMPLETED` report, one `SKIPPED` report, and one export row for the batch id.
...
Run two or more contenders when the demo supports it. Check this result: one `COMPLETED` report, one `SKIPPED` report, and one export row for the batch id. Then stop or release the current owner and run the next cycle to verify takeover.

## API by task {#api-by-task}

1. Read `DynamoDbScheduledExportRunner` first; it defines the application-facing coordination boundary.
2. Follow `DynamoDbExportDemo` to see client construction, contender setup, and result reporting.
3. Read `DynamoDbScheduledExportRunnerTest` to learn which ownership transitions the release promises.
...
The workshop integrates DynamoDB conditional writes with a runnable Gradle application. The example owns its local fixture for learning, but production code should inject the backend client and let the application lifecycle close it.
...
Review separate lock/export table names, AWS endpoint and credentials, `nodeId`, `lockName`, `waitTime`, and logical `leaseTime`. Use the same logical lock identity on every replica and different identities for unrelated jobs or environments.
...
DynamoDB TTL is asynchronous cleanup metadata, not the correctness clock. Lease comparison and conditional writes decide ownership.
...
- [Gradle build](../../../../examples/dynamodb-export/build.gradle.kts)
- [Main implementation](../../../../examples/dynamodb-export/src/main/kotlin/io/bluetape4k/leader/examples/dynamodbexport/DynamoDbScheduledExportRunner.kt)
- [Demo entry point](../../../../examples/dynamodb-export/src/main/kotlin/io/bluetape4k/leader/examples/dynamodbexport/DynamoDbExportDemo.kt)
- [Verification test](../../../../examples/dynamodb-export/src/test/kotlin/io/bluetape4k/leader/examples/dynamodbexport/DynamoDbScheduledExportRunnerTest.kt)
