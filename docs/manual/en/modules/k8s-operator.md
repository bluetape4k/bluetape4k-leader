...
id: "k8s-operator"
title: "Kubernetes operator workshop"
locale: "en"
kind: "example"
...
# Kubernetes operator election workshop

> Runnable release workshop · Kubernetes Lease with Spring scheduling · source pinned to `0.4.0`

## Problem {#problem}

Every operator replica keeps its scheduled tick, but only the pod holding the `cronjob-reconciler` Lease may run the custom-resource reconcile body.
...
Use this workshop when every operator replica keeps its scheduled tick, but only the pod holding the `cronjob-reconciler` Lease may run the custom-resource reconcile body. Start with the example unchanged, then replace only the workload while preserving the lock identity, bounded execution, and observable skip path.
...
- **Coordination backend:** Kubernetes Lease with Spring scheduling stores or enforces the leadership decision.
...
Do not stop at a green build. Check this observable result: standby ticks continue without workload, one pod reconciles, and another pod takes over after lease loss.
...
Run two or more contenders when the demo supports it. Check this result: standby ticks continue without workload, one pod reconciles, and another pod takes over after lease loss. Then stop or release the current owner and run the next cycle to verify takeover.

## API by task {#api-by-task}

1. Read `OperatorController` first; it defines the application-facing coordination boundary.
2. Follow `K8sOperatorApp` to see client construction, contender setup, and result reporting.
3. Read `OperatorControllerTest` to learn which ownership transitions the release promises.
...
The workshop integrates Kubernetes Lease with Spring scheduling with a runnable Gradle application. The example owns its local fixture for learning, but production code should inject the backend client and let the application lifecycle close it.
...
Review Lease namespace/name, pod identity, scheduler delay, RBAC verbs, Deployment replicas, and Kubernetes client settings. Use the same logical lock identity on every replica and different identities for unrelated jobs or environments.
...
A standby still runs the scheduler trigger. Keep the protected body behind the elector boundary and grant only the Lease RBAC verbs the controller needs.
...
[![k8s operator Architecture diagram](https://raw.githubusercontent.com/bluetape4k/bluetape4k-leader/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-architecture-01.png)](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-architecture-01.svg)

_Release README: [`examples/k8s-operator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/k8s-operator/README.md)_

### K8s operator flow diagram

[![K8s operator flow diagram](https://raw.githubusercontent.com/bluetape4k/bluetape4k-leader/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-flow-01.png)](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-flow-01.svg)

_Release README: [`examples/k8s-operator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/k8s-operator/README.md)_

### K8s Operator scenario diagram

[![K8s Operator scenario diagram](https://raw.githubusercontent.com/bluetape4k/bluetape4k-leader/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-scenario-01.png)](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-scenario-01.svg)

_Release README: [`examples/k8s-operator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/k8s-operator/README.md)_

### k8s operator Sequence Flow diagram

[![k8s operator Sequence Flow diagram](https://raw.githubusercontent.com/bluetape4k/bluetape4k-leader/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-sequence-01.png)](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/docs/images/readme-diagrams/examples-k8s-operator-sequence-01.svg)
...
- [Stable example README](../../../../examples/k8s-operator/README.md)
- [Gradle build](../../../../examples/k8s-operator/build.gradle.kts)
- [Main implementation](../../../../examples/k8s-operator/src/main/kotlin/io/bluetape4k/leader/examples/k8soperator/OperatorController.kt)
- [Demo entry point](../../../../examples/k8s-operator/src/main/kotlin/io/bluetape4k/leader/examples/k8soperator/K8sOperatorApp.kt)
- [Verification test](../../../../examples/k8s-operator/src/test/kotlin/io/bluetape4k/leader/examples/k8soperator/OperatorControllerTest.kt)
