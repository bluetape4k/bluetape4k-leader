...
`LeaderElectionErrorOverride` explicitly enables `lockName`. Status overrides use the same
allow-list. Backend messages, stack traces, and cause details are logged only as a sanitized
cause type and never copied into the response. `CancellationException` is rethrown.
...
`ktor-server-status-pages` artifact. Detached `leaderScheduled` exceptions are not routed
through HTTP: the plugin logs a sanitized `WARN`, skips the failed iteration, and continues.
