...
title: "Tenant aggregator workshop"
locale: "en"
kind: "example"
...
# Multi-tenant coroutine aggregator workshop

> Runnable release workshop · Exposed R2DBC suspend election · source pinned to `0.4.0`

## Problem {#problem}

Every replica starts one supervised coroutine per tenant, but a tenant-specific lock allows only one active aggregator for that tenant. Different tenants may elect different nodes.
...
Use this workshop when every replica starts one supervised coroutine per tenant, but a tenant-specific lock allows only one active aggregator for that tenant. Different tenants may elect different nodes. Start with the example unchanged, then replace only the workload while preserving the lock identity, bounded execution, and observable skip path.
...
- **Coordination backend:** Exposed R2DBC suspend election stores or enforces the leadership decision.
...
Do not stop at a green build. Check this observable result: each tenant has no concurrent aggregation violation, one tenant failure does not poison other loops, and graceful stop cancels all workers.
...
Run two or more contenders when the demo supports it. Check this result: each tenant has no concurrent aggregation violation, one tenant failure does not poison other loops, and graceful stop cancels all workers. Then stop or release the current owner and run the next cycle to verify takeover.

## API by task {#api-by-task}

1. Read `TenantAggregator` first; it defines the application-facing coordination boundary.
2. Follow `TenantAggregatorDemo` to see client construction, contender setup, and result reporting.
3. Read `TenantAggregatorTest` to learn which ownership transitions the release promises.
...
The workshop integrates Exposed R2DBC suspend election with a runnable Gradle application. The example owns its local fixture for learning, but production code should inject the backend client and let the application lifecycle close it.
...
Review tenant list, `lockNamePrefix`, `pollInterval`, wait/lease times, R2DBC dialect, application scope, and shutdown timeout. Use the same logical lock identity on every replica and different identities for unrelated jobs or environments.
...
Per-tenant locks provide at-least-once continuation after lease expiry, not exactly-once business effects. Aggregate writes need their own idempotency key.
...
### tenant aggregator Architecture diagram

[![tenant aggregator Architecture diagram](../../assets/readme-diagrams/examples-tenant-aggregator-architecture-01.png)](../../assets/readme-diagrams/examples-tenant-aggregator-architecture-01.svg)

_Release README: [`examples/tenant-aggregator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/tenant-aggregator/README.md)_

### Tenant aggregator flow diagram

[![Tenant aggregator flow diagram](../../assets/readme-diagrams/examples-tenant-aggregator-flow-01.png)](../../assets/readme-diagrams/examples-tenant-aggregator-flow-01.svg)

_Release README: [`examples/tenant-aggregator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/tenant-aggregator/README.md)_

### Tenant aggregator scenario diagram

[![Tenant aggregator scenario diagram](../../assets/readme-diagrams/examples-tenant-aggregator-scenario-01.png)](../../assets/readme-diagrams/examples-tenant-aggregator-scenario-01.svg)

_Release README: [`examples/tenant-aggregator/README.md`](https://github.com/bluetape4k/bluetape4k-leader/blob/17ab7f872c1f96318c73d3580729cac20a67e017/examples/tenant-aggregator/README.md)_

### tenant aggregator Sequence Flow diagram

[![tenant aggregator Sequence Flow diagram](../../assets/readme-diagrams/examples-tenant-aggregator-sequence-01.png)](../../assets/readme-diagrams/examples-tenant-aggregator-sequence-01.svg)
...
- [Stable example README](../../../../examples/tenant-aggregator/README.md)
- [Gradle build](../../../../examples/tenant-aggregator/build.gradle.kts)
- [Main implementation](../../../../examples/tenant-aggregator/src/main/kotlin/io/bluetape4k/leader/examples/tenant/TenantAggregator.kt)
- [Demo entry point](../../../../examples/tenant-aggregator/src/main/kotlin/io/bluetape4k/leader/examples/tenant/TenantAggregatorDemo.kt)
- [Verification test](../../../../examples/tenant-aggregator/src/test/kotlin/io/bluetape4k/leader/examples/tenant/TenantAggregatorTest.kt)
