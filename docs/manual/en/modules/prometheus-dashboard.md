...
title: "Prometheus dashboard workshop"
locale: "en"
kind: "example"
...
# Prometheus and Grafana operations workshop

> Runnable release workshop · Spring Boot, Lettuce Redis, Micrometer, Prometheus, and Grafana · source pinned to `0.4.0`

## Problem {#problem}

A proxied `@LeaderElection` job emits Micrometer counters, timers, and an active gauge. Actuator exposes them for Prometheus, and Grafana renders a pre-provisioned dashboard.
...
Use this workshop when a proxied `@LeaderElection` job emits Micrometer counters, timers, and an active gauge. Actuator exposes them for Prometheus, and Grafana renders a pre-provisioned dashboard. Start with the example unchanged, then replace only the workload while preserving the lock identity, bounded execution, and observable skip path.
...
- **Coordination backend:** Spring Boot, Lettuce Redis, Micrometer, Prometheus, and Grafana stores or enforces the leadership decision.
...
Do not stop at a green build. Check this observable result: `leader_aop_*` series appear, acquisition and skip rates change under contention, and the active gauge returns to zero.

## Quick start {#quick-start}

Prerequisites: JDK 21+; Docker for the compose stack or local Redis fallback.

```bash
./gradlew :examples:prometheus-dashboard:run
```

Run two or more contenders when the demo supports it. Check this result: `leader_aop_*` series appear, acquisition and skip rates change under contention, and the active gauge returns to zero. Then stop or release the current owner and run the next cycle to verify takeover.

## API by task {#api-by-task}

1. Read `PrometheusDashboardApp` first; it defines the application-facing coordination boundary.
2. Follow `PrometheusDashboardApp` to see client construction, contender setup, and result reporting.
3. Read `PrometheusScrapeTest` to learn which ownership transitions the release promises.
...
The workshop integrates Spring Boot, Lettuce Redis, Micrometer, Prometheus, and Grafana with a runnable Gradle application. The example owns its local fixture for learning, but production code should inject the backend client and let the application lifecycle close it.
...
Review Actuator exposure, Redis URL, lock name, metric tags, Prometheus scrape target, Grafana credentials, and host port bindings. Use the same logical lock identity on every replica and different identities for unrelated jobs or environments.
...
`leader_aop_active` is JVM-local; use `max by (lock_name)` across replicas instead of summing it. Metrics diagnose coordination but do not prove business-side idempotency.
...
- [Stable example README](../../../../examples/prometheus-dashboard/README.md)
- [Gradle build](../../../../examples/prometheus-dashboard/build.gradle.kts)
- [Main implementation](../../../../examples/prometheus-dashboard/src/main/kotlin/io/bluetape4k/leader/examples/prometheus/PrometheusDashboardApp.kt)
- [Verification test](../../../../examples/prometheus-dashboard/src/test/kotlin/io/bluetape4k/leader/examples/prometheus/PrometheusScrapeTest.kt)
