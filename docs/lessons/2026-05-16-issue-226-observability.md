...
Spring Boot 및 Ktor 사용자에게는 알려진 리더 선택 잠금에 대한 옵트인 상태 엔드포인트와 Spring `LeaderElectionEventPublisher` Bean 표면이 필요했습니다.

## 결정

백엔드 잠금을 열거하는 대신 JVM 로컬 잠금 이름 레지스트리와 엔드포인트 계층을 추가하세요. 정적 잠금 이름은 레지스트리를 시드하고 수신기 인식 선택기 또는 Ktor `leaderScheduled()` 호출은 관찰된 이름을 추가할 수 있습니다.
...
- 두 표면 모두 `auditLeaderId` 및 `leaseUntil`를 사용하여 잠금별 단일 지시선 상태를 반환합니다.
...
- 결과: `leader-ktor` 16 테스트 및 `leader-spring-boot` 293 테스트를 통과했습니다. 전체 실행은 기존 종료/로깅 스레드 경고를 인쇄했지만 success적으로 완료되었습니다.
