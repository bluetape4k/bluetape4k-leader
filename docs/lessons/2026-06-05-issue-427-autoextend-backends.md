...
이제 벤치마크 모듈에는 Local/MongoDB 자동 확장 행이 차단 및 일시 중단됩니다. 새로운 원시 JSON 및 해석 보고서는 `docs/benchmarks/2026-06-05-issue-427-autoextend-backends.md`에 보존됩니다.
...
Redis 임대 연장 결과를 #422에 연결해 유지합니다. 향후 자동 확장 벤치마크 격차에 대비해 먼저 README/공용 API가 지원되는 백엔드 또는 옵션 조합을 문서화하는지 검증하세요. 지원되지 않는 그룹 선택 자동 확장에는 벤치마크 행 이전에 API/지원 작업이 필요합니다.
