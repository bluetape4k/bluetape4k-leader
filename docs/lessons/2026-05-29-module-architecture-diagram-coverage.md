...
모든 모듈을 동일한 행/열 그리드에 강제로 배치하는 대신 입력, 자동 구성, 선출기, 백엔드 클라이언트, 백엔드 상태 및 내보내기를 관계별로 그룹화하는 보다 자유로운 모듈 수준 레이아웃을 사용하세요. `scripts/generate-module-architecture-diagrams.mjs`에서 모듈 아키텍처 SVG를 생성한 다음 공유 Graphviz 증거/PNG 재생성 게이트를 실행합니다.
