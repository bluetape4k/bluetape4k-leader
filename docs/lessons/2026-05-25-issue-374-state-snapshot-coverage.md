...
결정: 스냅샷 충실도는 백엔드마다 다르기 때문에 공유 계약 고정 장치 대신 백엔드별 통합 테스트를 추가합니다. Consul 및 DynamoDB 단일/그룹 스냅샷은 감사 ID, 물리적 노드 ID 및 리스 시간을 노출합니다. etcd 그룹 스냅샷은 백엔드 소유권 토큰과 슬롯을 노출하는 반면, etcd 단일 리더 `state()`는 소유자 메타데이터가 현재 구현에 의해 노출되지 않기 때문에 기본 빈 스냅샷으로 유지됩니다.
