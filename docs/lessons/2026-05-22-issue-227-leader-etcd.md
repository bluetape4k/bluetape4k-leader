...
Issue #227은 전체 etcd v3 리더 선택 백엔드에 대한 서사시입니다. 이 분기에서는 내구성 있는 디자인 아티팩트, 모듈 등록, 최초의 내부 jetcd 경계 유형, 공개 단일 리더 선택기, 공개 그룹 선택기, 감시 지원 이벤트 게시자 및 Spring Boot 공장 배선으로 구현을 시작했습니다.

## 결정

첫 번째 PR을 좁게 유지: `leader-etcd` 모듈을 추가하고, 활성 `bluetape4k-projects` `1.8.1-SNAPSHOT`에 대해 프로젝트 버전을 지정하고, 낮은 수준 임대/잠금 경계를 증명한 다음, 첫 번째 공개 단일 리더 선택자를 추가하는 동시에 그룹 선택, 감시 지원 이벤트 게시 및 Spring Boot 자동 구성을 제한된 후속 조각으로 추가합니다.

실제 통합 테스트에는 `bluetape4k-testcontainers` `EtcdServer.Launcher.etcd`를 사용하세요. 모의 기반 jetcd 테스트는 경계 위임에 유용하지만 서버 끝점 연결, 임대 유지 또는 etcd 잠금 대기열을 입증하지는 않습니다.

## 결과

- 설정 및 리더 BOM에 `leader-etcd`를 추가했습니다.
- etcd 옵션 모델, 키/경로 인코딩, 임대 타이밍/핸들 도우미, 백엔드 오류 분류 및 `JetcdEtcdLockClient` 경계가 추가되었습니다.
- jetcd Lock 임대로 지원되는 차단, 코루틴 및 가상 스레드 단일 리더 선택기를 추가했습니다.
- 슬롯당 하나의 jetcd 잠금 키로 지원되는 차단 및 코루틴 그룹 선택기를 추가했습니다.
- 구성된 접두사를 감시하고 etcd 소유권 키 `PUT`/`DELETE` 이벤트를 `Elected`/`Revoked`에 매핑하는 `EtcdLeaderElectionEventPublisher`를 추가했습니다.
- 호출자가 소유한 jetcd `Client` Bean이 있는 경우 차단, 코루틴 및 그룹 선택기에 대한 etcd Spring Boot 자동 구성을 추가했습니다.
...
- 잠금 획득 시간 초과 정리를 취소하여 임대 취소 전에 jetcd 잠금 미래를 보류 중으로 취소하여 시간 초과된 경쟁자가 보유자 뒤에 대기하지 않도록 했습니다.
...
백엔드 동작에 충분한 것으로 모의된 jetcd 테스트를 처리하지 마십시오. 나중에 추가되는 모든 공용 etcd 선택기에 대해 경합이 리더 계약 결과를 반환하고, 임대 만료로 소유권이 복구되고, 정리가 로컬 프로세스 상태에 의존하지 않음을 증명하는 일치하는 EtcdServer 지원 테스트를 포함합니다.
