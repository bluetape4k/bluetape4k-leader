...
the first internal jetcd boundary types, public single-leader electors, and
public group electors.
...
adding group election as the next bounded slice and deferring watch APIs plus
...
- Added real EtcdServer-backed integration tests matching the mock lock-client
  ...
  group max-leader bounds, slot reacquire, and active-lock extension behavior.
...
Latest test run executed 55 `leader-etcd` tests: 31 pure unit tests, 3 mocked
jetcd boundary tests, and 21 real EtcdServer integration tests.
