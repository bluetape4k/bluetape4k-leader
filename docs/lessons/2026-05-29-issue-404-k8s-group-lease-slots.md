# 강의 — 이슈 404 슬롯당 Kubernetes Lease 그룹 선출

날짜: 2026-05-29 이슈: #404 브랜치: feat/404-k8s-group-lease-slots

## 맥락

`leader-k8s`는 이미 획득당 펜싱 토큰을 사용하여 안전한 단일 리스 선택을 수행했지만 슬롯당 리스 모델이 소유자 조건부 릴리스 및 확장을 유지할 수 있을 때까지 그룹 선출은 제외되었습니다.

## 결정

`<lockName>-slot-<slotIndex>`라는 그룹 슬롯당 하나의 Kubernetes Lease를 사용합니다. 각 슬롯에 대해 `KubernetesLeaseLock`를 재사용하여 그룹 선출이 기존 리소스 버전 비교 및 ​​설정, 홀더 펜싱 토큰, 릴리스, 상태 매핑 및 확장 위임 동작을 상속합니다.
...
구현에서는 차단/비동기 및 정지 그룹 선출기, `LeaderGroupState`에 대한 슬롯 상태 매핑, 그룹 획득 및 정리 동작에 대한 K3s 적용 범위, 영어와 한국어로 된 README 문서를 추가합니다.
...
Kubernetes 그룹 선출의 경우 슬롯별 리스 소유권 검증에 대한 정확성을 유지합니다. `LeaderGroupState`는 관찰 가능성 스냅샷일 뿐이며 획득 게이트가 되어서는 안 됩니다.
