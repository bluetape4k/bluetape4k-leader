...
픽스쳐는 로컬 차단 및 정지 선택기를 사용하므로 백엔드 I/O가 주석 조회, 인수 검사, SpEL 평가, 코루틴 연속 처리 및 선택적 AOP 레코더 디스패치를
​​숨기지 않습니다.
...
차단 정적 이름 조언 경로는 직접 로컬 선택기 기준에 대해 주석 메타데이터 조회, AspectJ 조인 포인트 디스패치, Bean 선택 및 레코더 반복과 같은
