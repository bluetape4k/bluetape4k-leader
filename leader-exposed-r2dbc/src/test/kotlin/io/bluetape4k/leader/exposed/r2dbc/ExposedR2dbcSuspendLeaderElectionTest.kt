...
class ExposedR2dbcSuspendLeaderElectionTest: AbstractExposedR2dbcLeaderTest() {
    ...
    private suspend fun makeElection(testDB: TestR2dbcDB): ExposedR2dbcSuspendLeaderElection {
        ...
        return ExposedR2dbcSuspendLeaderElection(
            db,
    ...
    fun `runIfLeader - 동일 lockName에 이미 리더가 있으면 null을 반환한다`(testDB: TestR2dbcDB) = runSuspendIO {
        ...
        val holder = ExposedR2dbcSuspendLeaderElection(db, leaderOptions)
        ...
        val contender = ExposedR2dbcSuspendLeaderElection(db, contenderOptions)
    ...
    fun `runIfLeader - CancellationException 발생 시 락이 해제되어 재획득 가능하다`(testDB: TestR2dbcDB) = runSuspendIO {
        ...
        val result = ExposedR2dbcSuspendLeaderElection(db, options).runIfLeader(lockName) { "reacquired" }
