package io.bluetape4k.leader.spring.boot4.aop.autoconfigure
...
class LeaderAopAutoConfiguration {
    ...
    // [Boot 4 호환] HealthIndicator 는 Boot 4 에서 spring-boot-health 모듈로 이동
    // (org.springframework.boot.health.contributor.HealthIndicator). common 의 LeaderAopHealthIndicator
    // 는 Boot 3 path (actuate.health) 만 사용하므로 Boot 4 autoconfig 에서는 등록하지 않음.
    // Boot 4 health 통합은 후속 PR (별도 leader-spring-boot4-health 모듈 또는 공용 wrapper) 검토.
}
